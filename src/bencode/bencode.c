...
be_type be_get_type(const be_node_t *self) {
    ...
    if (self->len <= 0) return BE_NOT_VALID;
...
int be_read_string(be_node_t *self, char *str, size_t size) {
    ...
    if (self->len < 3) return BE_NOT_VALID;
...
int be_read_integer(be_node_t *self, int64_t *val) {
    ...
    if (self->buf[0] != 'i' || self->len < 3) return BE_NOT_VALID;

    if (self->buf[n] == '0' && self->buf[n + 1] == 'e') {
        *val = 0;
        if (n + 1 > self->len) self->type = BE_NOT_VALID;
        self->start = n + 2;
        return BE_OK;
    }
    ...
    if (self->buf[n] != 'e') return BE_NOT_VALID;

    if (n >= self->len) self->type = BE_NOT_VALID;
    self->start = n + 1;
